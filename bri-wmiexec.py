...
class WMIEXEC:
    def __init__(self, command='', username='', password='', domain='', hashes=None, aesKey=None, share=None,
        ...
        self.brinkles_command = command
        self.brinkles_username = username
        self.brinkles_password = password
        self.brinkles_domain = domain
        self.brinkles_lmhash = ''
        self.__nthash = ''
        self.brinkles_aesKey = aesKey
        self.brinkles_share = share
        self.brinkles_noOutput = noOutput
        self.brinkles_doKerberos = doKerberos
        self.brinkles_kdcHost = kdcHost
        self.brinkles_shell_type = shell_type
        self.shell = None
        if hashes is not None:
            self.brinkles_lmhash, self.__nthash = hashes.split(':')

    def run(self, addr, silentCommand=False):
        if self.brinkles_noOutput is False and silentCommand is False:
            smbConnection = SMBConnection(addr, addr)
            if self.brinkles_doKerberos is False:
                smbConnection.login(self.brinkles_username, self.brinkles_password, self.brinkles_domain, self.brinkles_lmhash, self.__nthash)
            else:
                smbConnection.kerberosLogin(self.brinkles_username, self.brinkles_password, self.brinkles_domain, self.brinkles_lmhash,
                                            self.__nthash, self.brinkles_aesKey, kdcHost=self.brinkles_kdcHost)
        ...
        dcom = DCOMConnection(addr, self.brinkles_username, self.brinkles_password, self.brinkles_domain, self.brinkles_lmhash, self.__nthash,
                              self.brinkles_aesKey, oxidResolver=True, doKerberos=self.brinkles_doKerberos, kdcHost=self.brinkles_kdcHost)
        try:
            ...
            self.shell = RemoteShell(self.brinkles_share, win32Process, smbConnection, self.brinkles_shell_type, silentCommand)
            if self.brinkles_command != ' ':
                self.shell.onecmd(self.brinkles_command)
            else:
                self.shell.cmdloop()
...
""")
    ...
    def get_output(self):
        ...
        while True:
            try:
                self.__transferClient.getFile(self.brinkles_share, self.__output, output_callback)
                break
            except Exception as e:
        ...
        self.__transferClient.deleteFile(self.brinkles_share, self.__output)
    ...
    def execute_remote(self, data, shell_type='cmd'):
        ...
        if self.brinkles_noOutput is False:
            command += ' 1> ' + '\\\\127.0.0.1\\%s' % self.brinkles_share + self.__output + ' 2>&1'
        if PY2:
